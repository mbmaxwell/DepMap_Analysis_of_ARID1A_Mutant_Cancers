...
Below I'm including the steps to download *ARID1A* mutant RNAi genetic dependencies data from DepMap portal:
...
To test if R-Loop regulator genes are over-represented among the 9 *ARID1A* mutant dependency genes, we can use a reference list of putative R-Loop regulators from a resource called [R-Loop Base](https://academic.oup.com/nar/article/50/D1/D303/6430826) plus genes that encode BAF/SWI complex subunits compared to the ~17,000 non depndency genes as a background list. 
...
Below I'm including the steps to download *ARID1A* mutant vs Non-Mutant CCLE Proteomics data from DepMap portal:
1) Navigate to  https://depmap.org/portal/
2) select "Tools" tab > custom analysis > Two Class Comparison
3) Upload list of *ARID1A* mutant DepMap cell lines (n=222) included in this repo ("ARID1A_Mutant_Cell_Lines_DepMap_ID)
